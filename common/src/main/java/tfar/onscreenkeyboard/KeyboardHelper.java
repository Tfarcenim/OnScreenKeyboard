...
public class KeyboardHelper {
    ...
    public static void addKeyboardDirectly(Screen screen, int xPoint, int yPoint, Consumer<Button> addButton, TextFieldHelper textFieldHelper){
        List<ShiftableButton> shiftableButtons = new ArrayList<>();
        addShiftableKeys(screen,xPoint,yPoint,addButton,shiftableButtons,textFieldHelper);
        addSpecialKeys(screen,xPoint,yPoint,addButton,shiftableButtons);
    ...
    public static void addShiftableKeys(Screen screen, int xPoint, int yPoint, Consumer<Button> consumer, List<ShiftableButton> shiftableButtons,TextFieldHelper textFieldHelper) {

        KeyboardLayout keyboardLayout = KeyboardLayout.QWERTY;

        makeShiftableRow(screen,xPoint,yPoint,keyboardLayout.row1(),consumer,shiftableButtons, textFieldHelper);
        makeShiftableRow(screen,xPoint,yPoint+22,keyboardLayout.row2(),consumer,shiftableButtons, textFieldHelper);
        makeShiftableRow(screen,xPoint,yPoint+44,keyboardLayout.row3(),consumer,shiftableButtons, textFieldHelper);
        makeShiftableRow(screen,xPoint+44,yPoint+66,keyboardLayout.row4(),consumer,shiftableButtons, textFieldHelper);
    }
    ...
    protected static void makeShiftableRow(Screen screen, int xStart, int yStart, List<CharCharPair> chars, Consumer<Button> addButton, List<ShiftableButton> shiftableButtons,TextFieldHelper textFieldHelper) {
        for (int i = 0 ; i < chars.size();i++) {
            CharCharPair charCharPair = chars.get(i);
            ShiftableButton shiftableButton = new ShiftableButton(screen.width / 2 +xStart + i * 22, screen.height / 2 +yStart, 20, 20, new TextComponent(String.valueOf(charCharPair.firstChar())), button -> {
                if (((ShiftableButton)button).shift) {
                    textFieldHelper.charTyped(charCharPair.secondChar());
                } else {
                    textFieldHelper.charTyped(charCharPair.firstChar());
                }
            },new TextComponent(String.valueOf(charCharPair.secondChar())));
            shiftableButtons.add(shiftableButton);
            addButton.accept(shiftableButton);
